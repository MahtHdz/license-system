...
SET MAIN_OBJ_OUTPUT_PATH=%PROJECT_PATH%obj\64Bits\
SET LIBS_OBJ_OUTPUT_PATH=%PROJECT_PATH%obj\64Bits\libs\
SET DLL_FLAGS= -shared -Wl,--output-def=%PROJECT_PATH%bin\64Bits\lib%DLL_NAME%.def -Wl,--out-implib=%PROJECT_PATH%bin\64Bits\lib%DLL_NAME%.a -Wl,--dll -luser32 
...
FOR /L %%i IN (0, 1, 4) DO (
    ...
    IF NOT "!FILES[%%i]!" == "main.cpp" (
        @!MINGW64_GCC_PATH!!FLAGS!!JNILIB_PATH!!JNILIB_WIN32_PATH!-c !PROJECT_PATH!\libs\!FILES[%%i]! -o !LIBS_OBJ_OUTPUT_PATH!!OBJS[%%i]! -m64
    ) ELSE (
        @!MINGW64_GCC_PATH!!FLAGS!!JNILIB_PATH!!JNILIB_WIN32_PATH!-c !PROJECT_PATH!!FILES[%%i]! -o !MAIN_OBJ_OUTPUT_PATH!!OBJS[%%i]! -m64
    )
    ECHO DONE.  
)
ECHO BUILDING DLL...
