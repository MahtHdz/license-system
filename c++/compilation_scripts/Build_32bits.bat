...
SET MINGW32_GCC_PATH="C:\Program Files\mingw-w64\x86_64-8.1.0-posix-seh-rt_v6-rev0\mingw64\bin\x86_64-w64-mingw32-g++.exe"
...
FOR /L %%i IN (0, 1, 4) DO (
    ...
    IF NOT "!FILES[%%i]!" == "main.cpp" (
        @!MINGW32_GCC_PATH!!FLAGS!!JNILIB_PATH!!JNILIB_WIN32_PATH!-c !PROJECT_PATH!libs\!FILES[%%i]! -o !LIBS_OBJ_OUTPUT_PATH!!OBJS[%%i]!
    ) ELSE (
        @!MINGW32_GCC_PATH!!FLAGS!!JNILIB_PATH!!JNILIB_WIN32_PATH!-c !PROJECT_PATH!!FILES[%%i]! -o !MAIN_OBJ_OUTPUT_PATH!!OBJS[%%i]!
    )
    ECHO DONE.  
)
ECHO BUILDING DLL...
@%MINGW32_GCC_PATH%%DLL_FLAGS% %OBJS_ABSOLUTE_PATHS[0]% %OBJS_ABSOLUTE_PATHS[1]% %OBJS_ABSOLUTE_PATHS[2]% %OBJS_ABSOLUTE_PATHS[3]% %OBJS_ABSOLUTE_PATHS[4]% -o %PROJECT_PATH%bin\32Bits\%DLL_NAME%.dll
XCOPY /s %PROJECT_PATH%bin\32Bits\%DLL_NAME%.dll %1\java\IDService\res
ECHO DONE.
ECHO. 
ECHO ###########################
